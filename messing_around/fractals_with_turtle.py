...
def iteration(bases, max_it, it, length):
    ...
    for i in range(3 * (4 ** it)):
        ...
        # vasnuft --- new_bases.append([x + math.cos(math.radians(bases[i][4] - 60)) * length / 3, y + math.sin(math.radians(bases[i][4] - 60)) * length / 3, x - math.cos(math.radians(bases[i][4] - 60)) * length / 3, ])
        print(new_bases)
